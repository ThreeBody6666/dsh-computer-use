	...
	factory: (require) => {
		...
		var CardForm = class {
			constructor(scope, specs) {
				...
				this.saving = false;
				this.failed = false;
				// The official settingsScope snapshot does not expose the `secrets`
				// list, so a write-only credential's "configured" badge is tracked
				// locally: a successful store marks it, a clear unmarks it.
				this.secretConfiguredLocal = new Set();
			...
			field(field) {
				...
				const snapshot = this.scope.getSnapshot();
				if (spec.secret === true) {
					const configured = this.secretConfigured(field);
			...
			async clear(field) {
				...
				if (spec.secret === true) {
					// Secret values are redacted out of the user layer, so a clear
					// can never be verified by reading it back. Trust the unset.
					this.secretConfiguredLocal.delete(field);
					return true;
				}
			...
			async store(field, value) {
				...
				if (spec.secret === true) {
					// Secret values never ride a response (the Host redacts them out
					// of the user layer), so a write cannot be verified by reading it
					// back. The mutate either settled ok or threw; trust it here.
					this.secretConfiguredLocal.add(field);
					return true;
				}
			...
			secretConfigured(field) {
				return this.secretConfiguredLocal.has(field);
			}
			publish() {
